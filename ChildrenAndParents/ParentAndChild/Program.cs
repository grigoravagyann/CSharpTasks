...
class Program
{
    static void Main()
    {
        ...
        {
            children[i] = new Child($"Child {i}", i, new Parent($"Father {i}", i + 30, i * 1000), new Parent($"Mother {i}", i + 25, i * 1000));
        }
        ChildrenMethods.PrintByParentsAge(70, children);
        ChildrenMethods.MaxAgeFatherSalary(children);
        ChildrenMethods.MaxFamilyIncomeChild(children);
        ChildrenMethods.SwapMinMaxAge(children);
    }
}